...
function ServicesSummary({ cart, total }) {
  ...
  return (
    <Box>
          ...
          <ListItem key={item.id} sx={{ py: 2, px: 0 }}>
            <ListItemText
              primary={
                <Typography
                  variant="subtitle1"
                  sx={{ fontWeight: 600, color: DARK_BLUE }}
                >
                  {item.name}
                  {getOptionDisplay(item) ? ` - ${getOptionDisplay(item)}` : ""}
                </Typography>
              }
              secondary={
                <Typography variant="body2" color="text.secondary">
                  £{item.pricePerItem} {item.priceType} × {item.quantity || 1}
                </Typography>
              }
            />
      ...
      >
        Your services have been added to your order. You can go back to modify
        your services or continue to schedule your collection and delivery.
      </Alert>
    </Box>
  );
}
